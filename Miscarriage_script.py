...
#mc['Miscarriage']= label_encoder.fit_transform(mc['Miscarriage']) 
#mc['Miscarriage'] = mc['Miscarriage'].cat.codes
...
# load data
#names = ['Age','Gender','Polyuria','Polydipsia','sudden weight loss','weakness','Polyphagia','Genital thrush','visual blurring','Itching','Irritability','delayed healing','partial paresis','muscle stiffness','Alopecia','Obesity','class']
# feature extraction
...
# Considering top 5 features for building a model
# Weight, Age, Race, diabetes and Previous miscarriage are considered
# Model is built using KNN
...
def get_user_input():
    ...
    Age = st.sidebar.slider('Your Age',18,49)
    Weight = st.sidebar.slider('Your Weight',45,79)
...
st.subheader('Miscarriage Calculator Parameters')
...
def visualize_confidence_level(prediction_proba):
    ...
    ax = grad_percentage.plot(kind='barh', figsize=(10, 7), color='#8B008B', zorder=10, width=0.5)
